...
for dhatu in dhatus[0:1]:
        ...
        for purusha in Purusha.choices():
            for vacana in Vacana.choices():
                ...
                print(lakara, purusha, vacana, len(prakriyas))
                '''
                for p in prakriyas:
                    print(p.text)
                '''
                ...
                #meaining = "Sorry, no meaning available for now"

                translit = lambda x: transliterate(str(x), Scheme.Slp1, Scheme.Iast)

                prompt = f'''You are an expert in Sanskrit grammar. You will conjugate Sanskrit verb roots according to Paninian rules.

                I will give you a Sanskrit dhātu (verb root) along with morphological markers also given in terms of their Sanskrit names. You must conjugate the verb correctly.
                ...
                chatgpt_answer = query_chatgpt(prompt)
                if chatgpt_answer:
                    ground_truth_iast = transliterate(ground_truth.text, Scheme.Slp1, Scheme.Iast)
